...
def get_latest_question_id() -> str:
    ...
    if not manifest_path.exists():
        raise FileNotFoundError("Manifest file not found. Please run step 1 first.")
...
def process_single_batch(client: Anthropic,
                         batch: List[Dict],
                         question: str,
    ...
    try:
        ...
        logger.debug(f"RAW model response (batch {batch_index+1}):\n{response_content}")
        ...
        if match:
            ...
            logger.warning(f"[yellow]No score found in response for batch {batch_index + 1}, using fallback score of 1[/yellow]")
        ...
        logger.error(f"[red]Error processing batch {batch_index + 1}: {e}[/red]")
    ...
    try:
        ...
        final_output = {
            ...
            "metadata": {
                ...
                "selection_criteria": f"Average score >= {MINIMUM_SCORE_THRESHOLD}",
        ...
        debug_output = {
            ...
            "metadata": {
                ...
                "successful_batches": sum(1 for r in all_responses if "error" not in r)
            }
        }
...
def main(question_id=None):
    try:
        console.print(Panel.fit("[yellow]Step 3: Final Passage Selection[/yellow]"))

        # Load environment / Anthropic
        api_key = os.getenv("ANTHROPIC_API_KEY")
        if not api_key:
            raise ValueError("[red]ANTHROPIC_API_KEY not found in environment variables[/red]")
        ...
        if not question:
            raise ValueError("[red]No question found in queried_results.json[/red]")
        ...
        with Progress(SpinnerColumn(),
            ...
            task_id = progress.add_task("[cyan]Processing batches...", total=len(batches))

            with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
                future_map = {
                    executor.submit(process_single_batch, client, batch, question, i): i
                ...
                for item in response["response"]:
                    ...
                    if ref and sc is not None:
                        try:
                            ...
                            passage_scores.setdefault(ref, []).append(numeric_score)
                        except ValueError:
                            logger.error(f"[red]Invalid score for {ref}: {sc}[/red]")

        averaged_scores = []
        for ref, scores in passage_scores.items():
            if scores:
                avg_score = sum(scores) / len(scores)
                averaged_scores.append({"reference": ref, "average_score": avg_score})

        # Filter by threshold
        filtered = [x for x in averaged_scores if x["average_score"] >= MINIMUM_SCORE_THRESHOLD]
        filtered.sort(key=lambda x: x["average_score"], reverse=True)

        top_passages = filtered[:TARGET_PASSAGES]
        logger.info(f"[green]Selected {len(top_passages)} passages >= {MINIMUM_SCORE_THRESHOLD}[/green]")
        ...
        for scored in top_passages:
            ...
            if ref in original_passages:
                ...
                logger.info(f"[cyan]Added passage {ref} => score {scored['average_score']}[/cyan]")
            else:
                logger.warning(f"[yellow]No original data found for {ref}[/yellow]")

        # Save or error
        if selected_passage_data:
            output_file = save_final_results(question_id, question, selected_passage_data, all_responses)
