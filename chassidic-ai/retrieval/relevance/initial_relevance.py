...
import instructor
from ...models.models import CandidatePassage, RelevanceJudgement
...
class PassageProposalBlock:
    def __init__(self, passages: list[CandidatePassage], text_proposal: str):
        self.passages: list[CandidatePassage] = passages
...
class RelevanceChecker:
    ...
    async def check_relevance(
        self, query: str, passages: list[CandidatePassage]
    ...
    def _passage_to_text_proposal(self, passage: CandidatePassage) -> str:
        # Prints an identified of the passage in a way which can be extracted and then the summary
        identifier = f"=== {passage.compound_id} ==="
        return f"{identifier}\n{passage.en_summary}"
    ...
    async def _with_limit(
        self, semaphore, query, passages_block: PassageProposalBlock
    ) -> list[CandidatePassage]:
        # Create an index from the passage id to the passage
        passage_id_to_passage = {p.compound_id: p for p in passages_block.passages}
...
"""
            ...
            # # Convert messages to chat completion message param
            # msgs: list[
            ...
            for result in response.relevance_results:
                ...
                if passage:
                    passage.relevance_judgments.append(
                        RelevanceJudgement(
                            passage_id=passage_id,
                            model="initial_relevance:gpt-4o-mini",
                            score=score,
                            reason=reason,
