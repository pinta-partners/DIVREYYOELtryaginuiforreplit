...
from retrieval.other_texts_datasource import (
    TextFromCSV,
    init_texts_from_csv_collection,
    get_all_texts,
)
from retrieval.vector_datasource import (
    VectorDocument,
    init_vector_indexes,
    insert_many,
    get_all_vectors,
...
async def main():
    ...
    source_collection = await init_texts_from_csv_collection(mongo_client)
    vector_collection = mongo_client.vectors.embeddings
    await init_vector_indexes(vector_collection)

    # Get all documents
    source_docs = await get_all_texts(source_collection)
    existing_vectors = await get_all_vectors(vector_collection)
    ...
    if new_docs:
        await insert_many(vector_collection, new_docs)
    ...
    if updated_docs:
        ...
        await vector_collection.bulk_write(operations)
