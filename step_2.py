...
def process_response_file_with_csv(input_json_path: Path, csv_file_path: Path, output_json_path: Path):
    ...
    try:
        # Load the JSON data
        logger.info(f"[cyan]Loading input JSON file from {input_json_path}...[/cyan]")
        with input_json_path.open("r", encoding="utf-8") as f:
            data = json.load(f)

        # Validate JSON structure
        if "answer" not in data or "relevant_passages" not in data["answer"]:
            raise ValueError("Invalid JSON structure. 'answer.relevant_passages' not found.")

        passages = data["answer"]["relevant_passages"]
        if not passages:
            raise ValueError("[ERROR] No relevant passages found in the JSON file.")
        ...
        for passage in passages:
            if not isinstance(passage, str):
                errors.append({"error": "Unsupported passage structure", "original": passage})
                continue
            ...
            try:
                ...
                if matched_row is not None and not matched_row.empty:
                    ...
                    matched_row = matched_row if isinstance(matched_row, pd.Series) else matched_row.iloc[0]
                ...
                logger.error(f"[red]Failed to process passage '{passage}'. Error: {e}[/red]")
                errors.append({"original": passage, "error": f"Processing error: {e}"})
        ...
        logger.info(f"[green]Queried results saved to {output_json_path}[/green]")
...
def main(question_id=None):
    try:
        console.print(Panel.fit("[yellow]Step 2: Processing Latest Question Results[/yellow]"))

        # Get the latest question ID from the manifest
        question_id = get_latest_question_id()
        logger.info(f"[cyan]Processing latest question ID: {question_id}[/cyan]")
        ...
        if not step_1_folder.exists():
            raise FileNotFoundError(f"Step 1 folder not found at '{step_1_folder}'")
        if not input_json_path.exists():
            raise FileNotFoundError(f"Response.json not found at '{input_json_path}'")
        if not csv_file_path.exists():
            raise FileNotFoundError(f"Dataset CSV not found at '{csv_file_path}'")

        # Process the response.json and save queried_results.json
        process_response_file_with_csv(input_json_path, csv_file_path, output_json_path)
        console.print(Panel.fit("[green]Processing completed successfully![/green]"))
